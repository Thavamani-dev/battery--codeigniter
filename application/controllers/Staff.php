...
class Staff extends CI_Controller {
    ...
    {
        $data['department']=$this->Orders_model->select_Orders();
    ...
    public function insert()
    {
        ...
        $this->form_validation->set_rules('txtUserName', 'UserName', 'required|is_unique[login_tbl.username]',array('is_unique'=>'Username already exists!'));
        ...
        if($this->form_validation->run() !== false)
        {
            ...
            $login=$this->Home_model->insert_login(array('username'=>$UserName,'password'=>$Password,'usertype'=>$role));
            if($login>0)
            {
              $data=$this->Staff_model->insert_staff(array('id'=>$login,'staff_name'=>$name,'gender'=>$gender,'email'=>$email,'mobile'=>$mobile,'dob'=>$dob,'doj'=>$doj,'address'=>$address,'status'=>$Status,'user_id'=>$role,'pic'=>$image,'added_by'=>$added));
